...
class Dataset:
    def __init__(self, batch_size, max_len):
        ...
        # Load the dataset
        self.data_train, self.info = tfds.load('ted_hrlr_translate/pt_to_en', split='train', as_supervised=True, with_info=True)
        ...
        self.data_train = self.setup_pipeline(self.data_train, is_training=True)
        self.data_valid = self.setup_pipeline(self.data_valid, is_training=False)
    ...
    def setup_pipeline(self, dataset, is_training):
        ...
        if is_training:
            # Shuffle the dataset while we have a training examples of 51785 
            dataset = dataset.shuffle(train_examples)

        # Batch and pad the dataset
        dataset = dataset.padded_batch(self.batch_size, padded_shapes=([None], [None]))
