...
class NST:
    ...
    def layer_style_cost(self, style_output, gram_target):
        ...
        if not (isinstance(gram_target, (tf.Tensor, tf.Variable))) or gram_target.shape.dims != [m, nc, nc]:
            raise TypeError(f'gram_target must be a tensor of shape [{m}, {nc}, {nc}]')
    ...
    def content_cost(self, content_output):
        if not (isinstance(content_output, (tf.Tensor, tf.Variable))) or content_output.shape.dims != self.content_feature.shape.dims:
    ...
    def variational_cost(image):
        if not (isinstance(image, (tf.Tensor, tf.Variable))) or not 3 <= image.shape.ndims <= 4:
            raise TypeError('image must be a tensor of rank 3 or 4')
        
        return tf.reduce_sum(tf.image.total_variation(image))

    def total_cost(self, generated_image):
        if not (isinstance(generated_image, (tf.Tensor, tf.Variable))) or generated_image.shape.dims != self.content_image.shape.dims:
    ...
    def generate_image(self, iterations=1000, step=None, lr=0.01, beta1=0.9, beta2=0.99):
        ...
        for i in range(iterations):
            ...
            if step is not None and i % step == 0:
                print(f"Cost at iteration {i}: {J}, content {J_content}, style {J_style}, var {J_var}")
            if J < best_cost:
                best_cost = J.numpy()
        ...
        if J < best_cost:
            ...
            best_image = generated_image.numpy()[0]
        if step is not None:
            print(f"Cost at iteration {iterations}: {J}, content {J_content}, style {J_style}, var {J_var}")
