...
class Decision_Tree():
    ...
    def Gini_split_criterion_one_feature(self,node,feature) :
        ...
        Gini_left          = 1-np.sum(np.square(np.sum(left_classes,axis=0)),axis=0)/(np.sum(filter_left,axis=0))/n
        right_classes      = np.logical_and(classes_ind[:,:,None],filter_right[:,None,:])
        Gini_right         = 1-np.sum(np.square(np.sum(right_classes,axis=0)),axis=0)/(np.sum(filter_right,axis=0))/n
